...
interface ResourceWeek {
  weekKey: string;
  totalPct: number;
  contributions: { planTitle: string; milestone: string; pct: number }[];
}
...
export default function ResourceAllocation() {
  ...
  const { data: plans, isLoading: plansLoading } = useQuery<JobPlan[]>({ queryKey: ["/api/job-plans"] });
  const { data: allLines, isLoading: linesLoading } = useQuery<JobPlanLine[]>({ queryKey: ["/api/job-plan-lines/all"] });

  const isLoading = plansLoading || linesLoading;

  const planMap = useMemo(() => new Map(plans?.map((p) => [p.id, p]) || []), [plans]);

  const earliestStart = useMemo(() => {
    if (!plans?.length) return new Date();
    const dates = plans.filter((p) => p.contractStartDate).map((p) => new Date(p.contractStartDate!));
    if (dates.length === 0) return new Date();
    return new Date(Math.min(...dates.map((d) => d.getTime())));
  }, [plans]);

  const weeks = useMemo(() => getWeekDates(earliestStart, numWeeks), [earliestStart, numWeeks]);
  ...
  const resources = useMemo(() => {
    ...
    allLines.forEach((line) => {
      const name = (line.resource || "").trim();
      if (!name) return;

      const plan = planMap.get(line.jobPlanId);
      const planTitle = plan?.title || "Unknown Plan";

      if (!byResource.has(name)) {
        byResource.set(name, {
          name,
          chargeOutLevel: line.chargeOutLevel || "",
          weeklyData: new Map(),
      ...
      const res = byResource.get(name)!;
      res.totalForecastHours += line.forecastHours || 0;
      res.plans.add(planTitle);

      let allocs: Record<string, number> = {};
      try { allocs = JSON.parse(line.weeklyAllocations || "{}"); } catch {}

      Object.entries(allocs).forEach(([weekKey, pct]) => {
        ...
        wd.totalPct += pct;
        wd.contributions.push({ planTitle, milestone: line.milestone, pct });
      });
    });

    const visibleKeys = new Set(weeks.map((w) => w.toISOString().split("T")[0]));
    byResource.forEach((res) => {
      res.overallocatedWeeks = [...res.weeklyData.entries()]
  ...
  const filteredResources = filterOveralloc ? resources.filter((r) => r.overallocatedWeeks > 0) : resources;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-full mx-auto space-y-5 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-sm text-muted-foreground">Cross-plan resource view with overallocation detection</p>
        </div>
              ...
              <table className="w-full text-sm" data-testid="table-resource-allocation">
                ...
                <tbody>
                  {filteredResources.map((res) => {
                    const isExpanded = expandedResources[res.name] || false;
                    return (
                      <ResourceRow
                        key={res.name}
                        resource={res}
                        ...
                        onToggle={() => toggleResource(res.name)}
                      />
                    );
                  })}
                </tbody>
...
function ResourceRow({
  ...
  return (
    <>
      <tr className={`border-b hover:bg-muted/10 cursor-pointer ${hasOveralloc ? "bg-destructive/[0.02]" : ""}`} onClick={onToggle} data-testid={`row-resource-${resource.name}`}>
            ...
            <div className="bg-muted/10 px-4 py-2 pl-10 space-y-1">
              ...
              {[...resource.plans].map((planTitle) => {
                ...
                resource.weeklyData.forEach((wd) => {
                  wd.contributions.forEach((c) => {
                    if (c.planTitle === planTitle) {
